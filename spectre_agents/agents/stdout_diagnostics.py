"""StdoutDiagnostics agent — MITgcm STDOUT failure diagnosis.

Ported from .claude/agents/mitgcm-stdout-diagnostics.md
"""
...
from spectre_agents.tools.slurm import job_status
...
### 2. Numerical blow-up
**Signature**: monitor stats show NaN, Inf, or exponentially growing values (T > 100C, CFL > 1e6)
...
- exf_hflux within -500 to +1600 W/m2
- exf_ustress/vstress within +/-2.0 N/m2 (clamped by windstressmax)
...
class StdoutDiagnostics(BaseSpectreAgent):
    name = "stdout_diagnostics"
    description = (
        "Parses MITgcm STDOUT files to diagnose run failures. "
        "Returns structured diagnosis with failure type and suggested fix."
    )
